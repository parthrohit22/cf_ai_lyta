...
test("canonical context preserves source versions and records non-sensitive selection manifests", async () => {
  ...
  await fixture.workspace.fetch(
    new Request("https://internal/library/search", {
      method: "POST",
...
test("deleting a source removes dependent context records while retaining ID-only audit history", async () => {
  ...
  const created = (await (
    await fixture.workspace.fetch(
      new Request("https://internal/context/records", {
        method: "POST",
        body: JSON.stringify({
          kind: "summary",
          ...
          provenance: {
            ...
            sourceVersionIds: ["source-version:immutable-hash-delete"],
            chunkIds: ["immutable-hash-delete-0"]
          }
        })
      })
    )
  ...
  await fixture.workspace.fetch(
    new Request("https://internal/library/search", {
      method: "POST",
      body: JSON.stringify({ queryVector: [1, 0], requestId: "request-delete" })
    })
  )
  const deleted = (await (
    await fixture.workspace.fetch(
      new Request("https://internal/library/delete", {
        method: "POST",
...
test("workspace context and manifests are isolated", async () => {
  ...
  await left.workspace.fetch(
    new Request("https://internal/library/search", {
      method: "POST",
      body: JSON.stringify({ queryVector: [1, 0], requestId: "left-request" })
    })
  )
  ...
  const rightManifests = (await (await right.workspace.fetch(new Request("https://internal/context/manifests"))).json()) as {
    manifests: unknown[]
  }
