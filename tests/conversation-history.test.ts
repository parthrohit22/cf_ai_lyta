...
test("canonical history preserves 20+ ordered messages when summary generation fails", async () => {
  ...
  const messages: ChatMessageRecord[] = Array.from({ length: 22 }, (_, index) => ({
    ...
    ...(index === 0 ? {
      attachments: [{
        id: "attachment-1", kind: "document", name: "notes.txt",
        mimeType: "text/plain", size: 8, extractedText: "source"
      }],
  ...
  assert.deepEqual(body.messages.map(message => message.id), messages.map(message => message.id))
